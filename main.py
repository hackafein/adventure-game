...
#Oyun pencere genisliği ve yüksekliğini ayarladık
screen_width = 1000
screen_height = 1000

# ekranı olusturduk ve pencere baslığını belirledik
...
dirt_img = dirt_img_list[random.randint(0,3)]
...
def reset_level(level):
	...
	# burası levele göre dünyayı pickle formatında yükleyip olusturacak kısım
...
class Player():
	...
	def update(self, game_over):
		...
		if game_over == 0:
			...
			# Yürüme animasyonunu burada olusturdum
			...
			# Çapısma var mı onu belirledim
			self.in_air = True
			for tile in world.tile_list:
				# burası x eksenindeki çarpısmaları kontrol eder
				if tile[1].colliderect(self.rect.x + dx, self.rect.y, self.width, self.height):
					dx = 0
				#burası y eksenindeki çarpısmaları kontrol eder
				if tile[1].colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
					#zıplama durumundaki çarpmanın sonucu düseriz
					if self.vel_y < 0:
						dy = tile[1].bottom - self.rect.top
						self.vel_y = 0
					#zemine düsersek dururuz
					elif self.vel_y >= 0:
			...
			#düsmanla olan çarpısmayı burada kontrol ediyorum
			if pygame.sprite.spritecollide(self, blob_group, False):
				game_over = -1
				game_over_fx.play()

			#lav ile olan çarpısmayı burada kontrol ediyorum
			if pygame.sprite.spritecollide(self, lava_group, False):
				game_over = -1
				game_over_fx.play()

			#çıkısa girip girmediğimi burada kontrol ediyorum
			if pygame.sprite.spritecollide(self, exit_group, False):
				game_over = 1


			#platormla olan çarpısmalar
...
#levellerin verisini yükle ve dünyayı olustur
if path.exists(f'level{level}_data'):
	pickle_in = open(f'level{level}_data', 'rb')
	world_data = pickle.load(pickle_in)
world = World(world_data)
...
while run:
	...
	if main_menu == True:
		...
		#eğer oyuncu leveli bitirdiyse kapıya ulastıysa
		if game_over == 1:
